...
export default function TransfersPage() {
  ...
  return <main className="app-shell"><AppSidebar active="transfers" /><section className="main-area transfer-page">
    ...
    <div className="content service-content">
      ...
      <section className="panel"><div className="section-heading"><div><h2>รายการขนย้ายเครื่องจักร</h2><p>แสดงต้นทาง–ปลายทางแยกแต่ละเครื่องจักร</p></div></div><div className="service-history-filters transfer-filters"><label>ค้นหา<input value={query} onChange={event => setQuery(event.target.value)} placeholder="หมายเลขเครื่องจักร / สถานที่ / วันที่" /></label><label>ปีงบประมาณ<select aria-label="กรองตามปีงบประมาณ" value={fiscalYear} onChange={event => setFiscalYear(event.target.value)}><option value="">ทุกปีงบประมาณ</option>{fiscalYears.map(year => <option key={year} value={year}>ปีงบประมาณ {year}</option>)}</select></label><label>เดือน<select aria-label="กรองตามเดือน" value={month} onChange={event => setMonth(event.target.value)}><option value="">ทุกเดือน</option>{thaiMonths.map((name, index) => <option key={name} value={index + 1}>{name}</option>)}</select></label><label>หน่วยงานต้นทาง<select aria-label="กรองตามหน่วยงานต้นทาง" value={fromDepartment} onChange={event => setFromDepartment(event.target.value)}><option value="">ทุกหน่วยงานต้นทาง</option>{fromDepartments.map(name => <option key={name} value={name}>{name}</option>)}</select></label><label>หน่วยงานปลายทาง<select aria-label="กรองตามหน่วยงานปลายทาง" value={toDepartment} onChange={event => setToDepartment(event.target.value)}><option value="">ทุกหน่วยงานปลายทาง</option>{toDepartments.map(name => <option key={name} value={name}>{name}</option>)}</select></label><button type="button" className="transfer-reset" onClick={resetFilters}>ล้างตัวกรอง</button></div>
