...
export default function TransfersPage() {
  ...
  return <main className="app-shell"><AppSidebar active="transfers" /><section className="main-area transfer-page">
    ...
    <div className="content service-content">
      ...
      <section className="panel"><div className="section-heading"><div><h2>รายการขนย้ายเครื่องจักร</h2><p>แสดงต้นทาง–ปลายทางแยกแต่ละเครื่องจักร</p></div></div><div className="service-history-filters rental-history-filters"><label>ค้นหา<input value={query} onChange={event => setQuery(event.target.value)} placeholder="หมายเลขเครื่องจักร / สถานที่ / วันที่" /></label><label>ปีงบประมาณ<select aria-label="กรองตามปีงบประมาณ" value={fiscalYear} onChange={event => setFiscalYear(event.target.value)}><option value="">ทุกปีงบประมาณ</option>{fiscalYears.map(year => <option key={year} value={year}>ปีงบประมาณ {year}</option>)}</select></label><label>เดือน<select aria-label="กรองตามเดือน" value={month} onChange={event => setMonth(event.target.value)}><option value="">ทุกเดือน</option>{thaiMonths.map((name, index) => <option key={name} value={index + 1}>{name}</option>)}</select></label><label>หน่วยงานต้นทาง<select aria-label="กรองตามหน่วยงานต้นทาง" value={fromDepartment} onChange={event => setFromDepartment(event.target.value)}><option value="">ทุกหน่วยงานต้นทาง</option>{fromDepartments.map(name => <option key={name} value={name}>{name}</option>)}</select></label><label>หน่วยงานปลายทาง<select aria-label="กรองตามหน่วยงานปลายทาง" value={toDepartment} onChange={event => setToDepartment(event.target.value)}><option value="">ทุกหน่วยงานปลายทาง</option>{toDepartments.map(name => <option key={name} value={name}>{name}</option>)}</select></label><button type="button" onClick={resetFilters}>ล้างตัวกรอง</button></div>
        <p className="rental-filter-summary" role="status">{loading ? "กำลังโหลดข้อมูล…" : `พบ ${filtered.length} จาก ${records.length} รายการขนย้าย`}</p><div className="table-wrap"><table><thead><tr><th>วันที่ขนย้าย</th><th>ขนย้ายโดย</th><th>เครื่องจักรที่ขนย้าย</th><th>จาก</th><th>ไป</th><th>จัดการ</th></tr></thead><tbody>{filtered.flatMap(record => record.items.map((item, index) => <tr key={`${record.id}-${index}`}>{index === 0 && <><td rowSpan={record.items.length}>{dateLabel(record.transferDate)}</td><td rowSpan={record.items.length}>{record.transporters.map(code => <div className="machine-code" key={code}>{code}</div>)}</td></>}<td><strong className="machine-code">{item.machineryCode}</strong></td><td>{item.from.name}{item.from.kind === "OTHER" && <span className="muted">อื่นๆ</span>}</td><td>{item.to.name}{item.to.kind === "OTHER" && <span className="muted">อื่นๆ</span>}</td>{index === 0 && <td rowSpan={record.items.length}><div className="service-row-actions"><button type="button" className="secondary compact-action" disabled={saving || loading || showForm} onClick={() => openForm(record)}>แก้ไข</button><ConfirmActionButton className="danger-button compact-action" title="ยืนยันลบรายการขนย้าย" message={`ลบรายการขนย้ายวันที่ ${dateLabel(record.transferDate)} โดย ${record.transporters.join(", ")} พร้อมเครื่องจักร ${record.items.length} รายการทั้งหมดในเที่ยวนี้? ไม่สามารถกู้คืนจากหน้าจอได้ หน่วยงานจะปรับตามประวัติล่าสุดที่เหลืออยู่ หากไม่มีประวัติจะคืนหน่วยงานก่อนเริ่มเชื่อมข้อมูล โดยไม่เปลี่ยนสถานะเครื่องจักรหรือประวัติระบบอื่น`} confirmLabel="ยืนยันลบข้อมูล" disabled={saving || loading || showForm} onConfirm={() => deleteRecord(record)}>ลบข้อมูล</ConfirmActionButton></div></td>}</tr>))}{!loading && !filtered.length && <tr><td colSpan={6} className="empty-state">{hasFilters ? "ไม่พบรายการขนย้ายตามตัวกรอง" : "ยังไม่มีประวัติขนย้าย กดบันทึกขนย้ายเพื่อเริ่มต้น"}</td></tr>}</tbody></table></div>
      </section>
    </div>
  </section></main>;
}
